...
import { createGTAQCODE } from "@gtaqcode/sdk"
...
`)
  ...
  const GTAQCODE = await createGTAQCODE({ port: 0 })
  ...
  try {
    ...
    const session = await GTAQCODE.client.session.create()
    const result = await GTAQCODE.client.session
      .prompt({
