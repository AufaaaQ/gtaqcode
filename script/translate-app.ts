...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "GTAQCODE/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".GTAQCODE/glossary/zh-cn.md"
  if (locale === "zht") return ".GTAQCODE/glossary/zh-tw.md"
  return `.GTAQCODE/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("GTAQCODE did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("GTAQCODE returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("GTAQCODE session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel GTAQCODE runs for 'all' (default: 4)
      --model <provider/id>  GTAQCODE model (default: GTAQCODE/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running GTAQCODE
...
`)
  ...
  if (failed.length) console.error(`\nGTAQCODE failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.GTAQCODE_DISABLE_PROJECT_CONFIG = "1"
  env.GTAQCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["GTAQCODE", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.GTAQCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["GTAQCODE", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.GTAQCODE_CONFIG
  delete env.GTAQCODE_CONFIG_DIR
  delete env.GTAQCODE_CONFIG_CONTENT
  delete env.GTAQCODE_PERMISSION
  delete env.GTAQCODE_AUTO_SHARE
  return env
}
