...
npm i -g GTAQCODE-ai@latest        # Î® bun/pnpm/yarn
scoop install GTAQCODE             # Windows
choco install GTAQCODE             # Windows
brew install anomalyco/tap/GTAQCODE # macOS ÎºÎ±Î¹ Linux (Ï€ÏÎ¿Ï„ÎµÎ¯Î½ÎµÏ„Î±Î¹, Ï€Î¬Î½Ï„Î± ÎµÎ½Î·Î¼ÎµÏÏ‰Î¼Î­Î½Î¿)
brew install GTAQCODE              # macOS ÎºÎ±Î¹ Linux (ÎµÏ€Î¯ÏƒÎ·Î¼Î¿Ï‚ Ï„ÏÏ€Î¿Ï‚ brew, Î»Î¹Î³ÏŒÏ„ÎµÏÎ¿ ÏƒÏ…Ï‡Î½Î­Ï‚ ÎµÎ½Î·Î¼ÎµÏÏŽÏƒÎµÎ¹Ï‚)
sudo pacman -S GTAQCODE            # Arch Linux (Î£Ï„Î±Î¸ÎµÏÏŒ)
paru -S GTAQCODE-bin               # Arch Linux (Î¤ÎµÎ»ÎµÏ…Ï„Î±Î¯Î± Î­ÎºÎ´Î¿ÏƒÎ· Î±Ï€ÏŒ AUR)
mise use -g GTAQCODE               # ÎŸÏ€Î¿Î¹Î¿Î´Î®Ï€Î¿Ï„Îµ Î»ÎµÎ¹Ï„Î¿Ï…ÏÎ³Î¹ÎºÏŒ ÏƒÏÏƒÏ„Î·Î¼Î±
nix run nixpkgs#GTAQCODE           # Î® github:anomalyco/GTAQCODE Î¼Îµ Î²Î¬ÏƒÎ· Ï„Î·Î½ Ï€Î¹Î¿ Ï€ÏÏŒÏƒÏ†Î±Ï„Î· Î±Î»Î»Î±Î³Î® Î±Ï€ÏŒ Ï„Î¿ dev branch
```
...
Î¤Î¿ GTAQCODE ÎµÎ¯Î½Î±Î¹ ÎµÏ€Î¯ÏƒÎ·Ï‚ Î´Î¹Î±Î¸Î­ÏƒÎ¹Î¼Î¿ Ï‰Ï‚ ÎµÏ†Î±ÏÎ¼Î¿Î³Î®. ÎšÎ±Ï„Î­Î²Î±ÏƒÎµ Ï„Î¿ Î±Ï€ÎµÏ…Î¸ÎµÎ¯Î±Ï‚ Î±Ï€ÏŒ Ï„Î· [ÏƒÎµÎ»Î¯Î´Î± ÎµÎºÎ´ÏŒÏƒÎµÏ‰Î½](https://github.com/anomalyco/GTAQCODE/releases) Î® Ï„Î¿ [GTAQCODE.ai/download](https://GTAQCODE.ai/download).
...
1. `$GTAQCODE_INSTALL_DIR` - Î ÏÎ¿ÏƒÎ±ÏÎ¼Î¿ÏƒÎ¼Î­Î½Î¿Ï‚ ÎºÎ±Ï„Î¬Î»Î¿Î³Î¿Ï‚ ÎµÎ³ÎºÎ±Ï„Î¬ÏƒÏ„Î±ÏƒÎ·Ï‚
...
4. `$HOME/.GTAQCODE/bin` - Î ÏÎ¿ÎµÏ€Î¹Î»ÎµÎ³Î¼Î­Î½Î· ÎµÏ†ÎµÎ´ÏÎ¹ÎºÎ® Î´Î¹Î±Î´ÏÎ¿Î¼Î®
...
Î¤Î¿ GTAQCODE Ï€ÎµÏÎ¹Î»Î±Î¼Î²Î¬Î½ÎµÎ¹ Î´ÏÎ¿ ÎµÎ½ÏƒÏ‰Î¼Î±Ï„Ï‰Î¼Î­Î½Î¿Ï…Ï‚ Ï€ÏÎ¬ÎºÏ„Î¿ÏÎµÏ‚ Î¼ÎµÏ„Î±Î¾Ï Ï„Ï‰Î½ Î¿Ï€Î¿Î¯Ï‰Î½ Î¼Ï€Î¿ÏÎµÎ¯Ï„Îµ Î½Î± ÎµÎ½Î±Î»Î»Î¬ÏƒÏƒÎµÏƒÏ„Îµ Î¼Îµ Ï„Î¿ Ï€Î»Î®ÎºÏ„ÏÎ¿ `Tab`.
...
ÎœÎ¬Î¸ÎµÏ„Îµ Ï€ÎµÏÎ¹ÏƒÏƒÏŒÏ„ÎµÏÎ± Î³Î¹Î± Ï„Î¿Ï…Ï‚ [Ï€ÏÎ¬ÎºÏ„Î¿ÏÎµÏ‚](https://GTAQCODE.ai/docs/agents).

### ÎŸÎ´Î·Î³ÏŒÏ‚ Î§ÏÎ®ÏƒÎ·Ï‚

Î“Î¹Î± Ï€ÎµÏÎ¹ÏƒÏƒÏŒÏ„ÎµÏÎµÏ‚ Ï€Î»Î·ÏÎ¿Ï†Î¿ÏÎ¯ÎµÏ‚ ÏƒÏ‡ÎµÏ„Î¹ÎºÎ¬ Î¼Îµ Ï„Î· ÏÏÎ¸Î¼Î¹ÏƒÎ· Ï„Î¿Ï… GTAQCODE, [**Ï€Î»Î¿Î·Î³Î®ÏƒÎ¿Ï… ÏƒÏ„Î¿Î½ Î¿Î´Î·Î³ÏŒ Ï‡ÏÎ®ÏƒÎ·Ï‚ Î¼Î±Ï‚**](https://GTAQCODE.ai/docs).

### Î£Ï…Î½ÎµÎ¹ÏƒÏ†Î¿ÏÎ¬

Î•Î¬Î½ ÎµÎ½Î´Î¹Î±Ï†Î­ÏÎµÏƒÎ±Î¹ Î½Î± ÏƒÏ…Î½ÎµÎ¹ÏƒÏ†Î­ÏÎµÎ¹Ï‚ ÏƒÏ„Î¿ GTAQCODE, Î´Î¹Î±Î²Î¬ÏƒÏ„Îµ Ï„Î± [Î¿Î´Î·Î³ÏŒ Ï‡ÏÎ®ÏƒÎ·Ï‚ ÏƒÏ…Î½ÎµÎ¹ÏƒÏ†Î¿ÏÎ¬Ï‚](./CONTRIBUTING.md) Ï€ÏÎ¹Î½ Ï…Ï€Î¿Î²Î¬Î»ÎµÎ¹Ï‚ Î­Î½Î± pull request.

### Î”Î·Î¼Î¹Î¿Ï…ÏÎ³Î¯Î± Ï€Î¬Î½Ï‰ ÏƒÏ„Î¿ GTAQCODE

Î•Î¬Î½ ÎµÏÎ³Î¬Î¶ÎµÏƒÎ±Î¹ ÏƒÎµ Î­Î½Î± Î­ÏÎ³Î¿ ÏƒÏ‡ÎµÏ„Î¹ÎºÏŒ Î¼Îµ Ï„Î¿ GTAQCODE ÎºÎ±Î¹ Ï‡ÏÎ·ÏƒÎ¹Î¼Î¿Ï€Î¿Î¹ÎµÎ¯Ï„Îµ Ï„Î¿ "GTAQCODE" Ï‰Ï‚ Î¼Î­ÏÎ¿Ï‚ Ï„Î¿Ï… Î¿Î½ÏŒÎ¼Î±Ï„ÏŒÏ‚ Ï„Î¿Ï…, Î³Î¹Î± Ï€Î±ÏÎ¬Î´ÎµÎ¹Î³Î¼Î± "GTAQCODE-dashboard" Î® "GTAQCODE-mobile", Ï€ÏÏŒÏƒÎ¸ÎµÏƒÎµ Î¼Î¹Î± ÏƒÎ·Î¼ÎµÎ¯Ï‰ÏƒÎ· ÏƒÏ„Î¿ README ÏƒÎ±Ï‚ Î³Î¹Î± Î½Î± Î´Î¹ÎµÏ…ÎºÏÎ¹Î½Î¯ÏƒÎµÎ¹Ï‚ ÏŒÏ„Î¹ Î´ÎµÎ½ ÎµÎ¯Î½Î±Î¹ ÎºÎ±Ï„Î±ÏƒÎºÎµÏ…Î±ÏƒÎ¼Î­Î½Î¿ Î±Ï€ÏŒ Ï„Î·Î½ Î¿Î¼Î¬Î´Î± Ï„Î¿Ï… GTAQCODE ÎºÎ±Î¹ Î´ÎµÎ½ Î­Ï‡ÎµÎ¹ ÎºÎ±Î¼Î¯Î± ÏƒÏ‡Î­ÏƒÎ· Î¼Îµ ÎµÎ¼Î¬Ï‚.

---

**Î“Î¯Î½Îµ Î¼Î­Î»Î¿Ï‚ Ï„Î·Ï‚ ÎºÎ¿Î¹Î½ÏŒÏ„Î·Ï„Î¬Ï‚ Î¼Î±Ï‚** [Discord](https://discord.gg/GTAQCODE) | [X.com](https://x.com/GTAQCODE)
