# Remove `packages/GTAQCODE/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/GTAQCODE/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the GTAQCODE legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/GTAQCODE/src/storage/db.ts` are concentrated in 22 source files:

- `packages/GTAQCODE/src/account/repo.ts`
- `packages/GTAQCODE/src/cli/cmd/db.ts`
- `packages/GTAQCODE/src/cli/cmd/import.ts`
- `packages/GTAQCODE/src/cli/cmd/stats.ts`
- `packages/GTAQCODE/src/control-plane/workspace.ts`
- `packages/GTAQCODE/src/index.ts`
- `packages/GTAQCODE/src/node.ts`
- `packages/GTAQCODE/src/permission/index.ts`
- `packages/GTAQCODE/src/project/project.ts`
- `packages/GTAQCODE/src/server/projectors.ts`
- `packages/GTAQCODE/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/GTAQCODE/src/server/shared/fence.ts`
- `packages/GTAQCODE/src/session/message-v2.ts`
- `packages/GTAQCODE/src/session/projectors.ts`
- `packages/GTAQCODE/src/session/prompt.ts`
- `packages/GTAQCODE/src/session/session.ts`
- `packages/GTAQCODE/src/session/todo.ts`
- `packages/GTAQCODE/src/share/share-next.ts`
- `packages/GTAQCODE/src/storage/db.ts`
- `packages/GTAQCODE/src/sync/index.ts`
- `packages/GTAQCODE/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy GTAQCODE database wrapper; `packages/GTAQCODE/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the GTAQCODE wrapper.
...
Status: Completed. `SyncEvent` and the GTAQCODE projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy GTAQCODE database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy GTAQCODE database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy GTAQCODE database wrapper.
...
All migration groups are complete or superseded. `packages/GTAQCODE/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No GTAQCODE data-migration group remains.

The previous GTAQCODE `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate GTAQCODE data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/GTAQCODE`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/GTAQCODE/src`
- `bun typecheck` from `packages/GTAQCODE`
- Relevant package tests from `packages/GTAQCODE`, not the repo root
