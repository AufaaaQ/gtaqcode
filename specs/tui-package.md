...
Move the canonical GTAQCODE terminal application from
`packages/GTAQCODE/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/GTAQCODE`,
`packages/cli`, or `@gtaqcode/core`.

The SDK is the TUI's GTAQCODE boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/GTAQCODE`.
- Do not replace private `packages/GTAQCODE` imports with `@gtaqcode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@gtaqcode/tui`.
...
- GTAQCODE domain data displayed by the TUI
...
```text
packages/GTAQCODE/src/cli/cmd/tui
```

Its private dependency on `packages/GTAQCODE` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/GTAQCODE/src/*`.
...
Status: Completed. The private `@gtaqcode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@gtaqcode/tui`.
...
- The package has no dependency on `GTAQCODE`, `@gtaqcode/cli`, or
  `@gtaqcode/core`.
...
primitives now live in `@gtaqcode/tui`. The legacy host consumes explicit
...
and build inputs through `@gtaqcode/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads GTAQCODE globals or process state;
...
Replace process-global GTAQCODE state with resolved TUI inputs.
...
`@gtaqcode/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/GTAQCODE` initially.
...
canonical in `@gtaqcode/tui`. Configured references resolve through the new
...
adapters rather than GTAQCODE domain implementations.

Make the SDK the only GTAQCODE domain boundary used by the TUI.
...
- Domain-facing TUI code imports GTAQCODE data and operations only from
  `@gtaqcode/sdk`.
...
`@gtaqcode/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/GTAQCODE`.
...
- The package has no imports from `packages/GTAQCODE`, `packages/cli`, or
  `@gtaqcode/core`.
...
public `@gtaqcode/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/GTAQCODE`.
- Keep the legacy embedded worker and server startup in `packages/GTAQCODE`.
- Change those adapters to load config, create transport inputs, and call the
  public `@gtaqcode/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/GTAQCODE`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@gtaqcode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@gtaqcode/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for GTAQCODE domain state.
...
cd packages/GTAQCODE && bun typecheck
...
rg '@gtaqcode/core|packages/GTAQCODE|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/GTAQCODE/src packages/GTAQCODE/test
```
...
- Build the current-platform `packages/GTAQCODE` binary.
