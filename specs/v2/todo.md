...
The GTAQCODE server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@gtaqcode/llm` tool loop after replacing its
...
- plugins get global "GTAQCODE" instance like in that post i showed
- GTAQCODE instance has stuff like `GTAQCODE.session.prompt()` or
  `GTAQCODE.tool.register({...})`
...
- decide whether to preserve deprecated `@gtaqcode/llm` orchestration exports
