...
Both `action` and `resource` use GTAQCODE's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.GTAQCODE` files is intentionally deferred until `.GTAQCODE` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether GTAQCODE permits an operation through its managed execution paths.
