...
> current private `@gtaqcode/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@gtaqcode/ai`
...
- Secondary audience: GTAQCODE and other durable agent runtimes
...
import { LLM } from "@gtaqcode/ai"
import { OpenAI } from "@gtaqcode/ai/providers/openai"
...
```ts
import { Message } from "@gtaqcode/ai"
...
import { LLM, Tool } from "@gtaqcode/ai"
...
GTAQCODE and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@gtaqcode/ai/promise"
import { OpenAI } from "@gtaqcode/ai/providers/openai"
...
```ts
import { LLMSchema } from "@gtaqcode/ai/schema"
...
```ts
import { Provider, Protocol } from "@gtaqcode/ai/provider"
...
@gtaqcode/ai/provider
  experimental Provider and Protocol authoring APIs

@gtaqcode/ai/providers/openai
@gtaqcode/ai/providers/anthropic
@gtaqcode/ai/providers/google
...
```
...
GTAQCODE should migrate to `generateTurn` / `streamTurn`, preserving its durable
