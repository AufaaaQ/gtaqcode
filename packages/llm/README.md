...
Schema-first LLM core for GTAQCODE. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@gtaqcode/llm"
import { OpenAI } from "@gtaqcode/llm/providers"
...
The math justifies the default: Anthropic's 5-minute cache write is 1.25Ã— base, read is 0.1Ã—, so a single reuse within 5 minutes already wins. One-shot completions below the per-model minimum-cacheable-token threshold silently no-op on the wire, so the worst case is harmless.
...
```ts
import { OpenAI, CloudflareAIGateway } from "@gtaqcode/llm/providers"
