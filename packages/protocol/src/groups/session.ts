...
import { PromptInput } from "@gtaqcode/schema/prompt-input"
import { Session } from "@gtaqcode/schema/session"
import { Project } from "@gtaqcode/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@gtaqcode/schema/schema"
...
import { SessionEvent } from "@gtaqcode/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this GTAQCODE process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this GTAQCODE process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
