...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "GTAQCODE.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "GTAQCODE.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "GTAQCODE.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "GTAQCODE.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["GTAQCODE.json", "GTAQCODE.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "GTAQCODE.json"),
        path.join(project, "GTAQCODE.jsonc"),
      ])
    })
  })
