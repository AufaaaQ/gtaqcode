...
describe("GTAQCODE acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ GTAQCODE }))
        ...
        expect(initialized.agentInfo?.name).toBe("GTAQCODE")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ GTAQCODE })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("GTAQCODE-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "GTAQCODE-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.GTAQCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("GTAQCODE-login")
