...
test("turn summary starts at the left edge", async () => {
  ...
  try {
    ...
    try {
      expect(renderRows(commits.at(-1)!)[0]).toBe("â–£ Build Â· Little Frank Â· 2.2s")
    } finally {
      destroy(commits)
    }
  } finally {
...
test("inserts spacers for new visible groups", async () => {
  ...
  try {
    ...
    try {
      ...
      expect(renderCommit(commits[1]!).trim()).toBe("â€º use subagent to explore run.ts")
    } finally {
      destroy(commits)
    }
  } finally {
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    ...
    try {
      ...
      expect(output).toContain("â€º Hello you")
...
test("renders plain errors with one blank line before and after the error block", async () => {
  ...
  try {
    ...
    const output = lines.join("\n")
    expect(output).toContain("â€º /fmt error\n\ndemo error event")
