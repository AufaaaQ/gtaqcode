// Subprocess integration tests for `GTAQCODE run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness â€” each test uses
// `GTAQCODE.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `GTAQCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("GTAQCODE run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const result = yield* GTAQCODE.run("say hi")
        GTAQCODE.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const result = yield* GTAQCODE.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* GTAQCODE.run("think", { extraArgs: ["--thinking"] })
        GTAQCODE.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* GTAQCODE.run("think again")
        GTAQCODE.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const result = yield* GTAQCODE.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const result = yield* GTAQCODE.run("say hi", { format: "json" })
        GTAQCODE.expectExit(result, 0)

        const events = GTAQCODE.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const result = yield* GTAQCODE.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const result = yield* GTAQCODE.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const result = yield* GTAQCODE.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* GTAQCODE.run("request permission", { permission: { bash: "ask" } })
        GTAQCODE.expectExit(denied, 0)
        ...
        const allowed = yield* GTAQCODE.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        GTAQCODE.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* GTAQCODE.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        GTAQCODE.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, GTAQCODE }) =>
      Effect.gen(function* () {
        ...
        const server = yield* GTAQCODE.serve()

        const result = yield* GTAQCODE.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, GTAQCODE }) =>
      Effect.gen(function* () {
        const result = yield* GTAQCODE.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, GTAQCODE }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* GTAQCODE.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
