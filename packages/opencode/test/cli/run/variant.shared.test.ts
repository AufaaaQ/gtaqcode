...
describe("run variant shared", () => {
  ...
  test("formats model labels", () => {
    expect(formatModelLabel(model, undefined)).toBe("gpt-5 Â· openai")
    expect(formatModelLabel(model, "high")).toBe("gpt-5 Â· openai Â· high")
    expect(formatModelLabel(model, undefined, providers)).toBe("GPT-5 Â· OpenAI")
    expect(formatModelLabel(model, "high", providers)).toBe("GPT-5 Â· OpenAI Â· high")
  })
