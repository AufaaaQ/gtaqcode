...
import { GTAQCODEClient, type GlobalEvent } from "@gtaqcode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<GTAQCODEClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<GTAQCODEClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<GTAQCODEClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<GTAQCODEClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<GTAQCODEClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://GTAQCODE.test"),
...
function sdk(
  input: {
    ...
    subscribe?: GTAQCODEClient["event"]["subscribe"]
    globalEvent?: GTAQCODEClient["global"]["event"]
    promptAsync?: GTAQCODEClient["session"]["promptAsync"]
    status?: GTAQCODEClient["session"]["status"]
    messages?: GTAQCODEClient["session"]["messages"]
    children?: GTAQCODEClient["session"]["children"]
    permissions?: GTAQCODEClient["permission"]["list"]
    questions?: GTAQCODEClient["question"]["list"]
  } = {},
) {
  const client = new GTAQCODEClient()

  const subscribe: GTAQCODEClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: GTAQCODEClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: GTAQCODEClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: GTAQCODEClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: GTAQCODEClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: GTAQCODEClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: GTAQCODEClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: GTAQCODEClient["question"]["list"] = input.questions ?? (() => ok([]))
