...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ GTAQCODE }) =>
    Effect.gen(function* () {
      const result = yield* GTAQCODE.spawn(["--replay-limit", "10"])

      GTAQCODE.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ GTAQCODE }) =>
    Effect.gen(function* () {
      const result = yield* GTAQCODE.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ GTAQCODE }) =>
    Effect.gen(function* () {
      const result = yield* GTAQCODE.spawn(["--mini", "--port", "4096"])
