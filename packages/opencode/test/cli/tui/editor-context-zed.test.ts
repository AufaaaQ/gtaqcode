...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@gtaqcode/tui/editor-zed"
...
test("offsetToPosition converts Zed offsets to 1-based editor positions", () => {
  ...
  expect(offsetToPosition("Ð–\nabc", utf8ByteOffset("Ð–\nabc", "Ð–\nabc".indexOf("a")))).toEqual({
    line: 2,
    character: 1,
  })
  expect(offsetToPosition("ðŸ˜€\nabc", utf8ByteOffset("ðŸ˜€\nabc", "ðŸ˜€\nabc".indexOf("a")))).toEqual({
    line: 2,
    character: 1,
  })
})
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.GTAQCODE_ZED_DB
  process.env.GTAQCODE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.GTAQCODE_ZED_DB
    else process.env.GTAQCODE_ZED_DB = previous
    home.mockRestore()
  }
})
...
test("resolveZedSelection handles non-ASCII text inside the selected range", async () => {
  await using tmp = await tmpdir()
  const contents = "a\npre\nÐ²Ñ‹Ð±Ð¾Ñ€\nz"
  const start = contents.indexOf("Ð²Ñ‹Ð±Ð¾Ñ€")
  const fixture = await writeZedFixture(tmp.path, {
    contents,
    selectionStart: utf8ByteOffset(contents, start),
    selectionEnd: utf8ByteOffset(contents, start + "Ð²Ñ‹Ð±Ð¾Ñ€".length),
  })
