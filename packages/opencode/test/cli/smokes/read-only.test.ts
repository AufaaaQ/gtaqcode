...
describe("GTAQCODE read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // GTAQCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["mcp", "list"])
        GTAQCODE.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["providers", "list"])
        GTAQCODE.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["models"])
        GTAQCODE.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["agent", "list"])
        GTAQCODE.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh GTAQCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["session", "list"])
        GTAQCODE.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["stats"])
        GTAQCODE.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under GTAQCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form â€” both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const r = yield* GTAQCODE.spawn(["db", "path"])
        GTAQCODE.expectExit(r, 0, "db path")
