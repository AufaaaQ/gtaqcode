...
// terminal sizes. The default GTAQCODE tui command is excluded â€”
// `GTAQCODE --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `GTAQCODE --help` prints today;
...
// top-level help on `--help` and exits 1; not a real GTAQCODE command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("GTAQCODE CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ GTAQCODE }) =>
      Effect.gen(function* () {
        const topLevel = yield* GTAQCODE.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* GTAQCODE.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`GTAQCODE ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`GTAQCODE ${argv.join(" ")} --help`)
        }
