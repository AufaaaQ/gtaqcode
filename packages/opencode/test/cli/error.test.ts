...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/GTAQCODE.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/GTAQCODE.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/GTAQCODE.jsonc", dir: ".GTAQCODE", suggestion: "GTAQCODE" },
        expected:
          'Directory ".GTAQCODE" in /tmp/GTAQCODE.jsonc is not valid. Rename the directory to "GTAQCODE" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/GTAQCODE.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/GTAQCODE.jsonc: schema mismatch\nâ†³ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.GTAQCODE.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.GTAQCODE.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `GTAQCODE models` to list available models",
      "Or check your config (GTAQCODE.json) provider/model names",
    ].join("\n")
