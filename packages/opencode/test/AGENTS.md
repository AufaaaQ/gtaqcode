...
- `config?: Partial<Config.Info>` - Write an `GTAQCODE.json` config file
...
- Directories are created in the system temp folder with prefix `GTAQCODE-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("GTAQCODE-test-")
  }),
)
```
...
When a test only needs to override one or two methods of a service, prefer `Layer.mock` over a hand-rolled `Layer.succeed(Service, Service.of({ ... }))`. `Layer.mock` lets you supply just the methods that matter â€” anything else throws an `UnimplementedError` defect if the test accidentally calls it, which is exactly the signal you want.
...
- `pollWithTimeout(effect, message, duration?)` from `test/lib/effect.ts` â€” repeatedly run a predicate effect until it returns a non-`undefined` value, with a timeout.
- `awaitWithTimeout(effect, message, duration?)` from `test/lib/effect.ts` â€” wrap any effect with `Effect.timeoutOrElse` and a custom error message.
- `llm.wait(n)` from `test/lib/llm-server.ts` â€” wait until the mock LLM has received `n` HTTP calls.
- `SessionStatus.Service` `.get(sessionID)` â€” observable per-session state (`{ type: "busy" | "idle" | ... }`).
- `BackgroundJob.wait({ id, timeout })` from `src/background/job.ts` â€” wait for a background job to complete.
- Bus subscriptions â€” fork `Stream.runForEach(bus.subscribe(Event), ...)` and open a `Latch` inside the callback to signal first-event readiness.
...
// Fix â€” wait for a published readiness signal
yield * prompt.shell({ command: "sleep 30" }).pipe(Effect.forkChild)
yield *
  pollWithTimeout(
    Effect.gen(function* () {
