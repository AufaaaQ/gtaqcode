...
it.instance(
  ...
  withTrackedSnapshot(({ tmp, snapshot, before }) =>
    Effect.gen(function* () {
      const unicodeFiles = [
        { path: fwd(tmp.path, "æ–‡ä»¶.txt"), content: "chinese content" },
        { path: fwd(tmp.path, "ðŸš€rocket.txt"), content: "emoji content" },
        { path: fwd(tmp.path, "cafÃ©.txt"), content: "accented content" },
        { path: fwd(tmp.path, "Ñ„Ð°Ð¹Ð».txt"), content: "cyrillic content" },
      ]
      yield* Effect.all(
...
it.instance.skip(
  ...
  Effect.gen(function* () {
    const tmp = yield* bootstrap()
    const snapshot = yield* Snapshot.Service
    const chineseFile = fwd(tmp.path, "æ–‡ä»¶.txt")
    const cyrillicFile = fwd(tmp.path, "Ñ„Ð°Ð¹Ð».txt")
...
it.instance(
  ...
  withTrackedSnapshot(({ tmp, snapshot, before }) =>
    Effect.gen(function* () {
      yield* mkdirp(`${tmp.path}/ç›®å½•/Ð¿Ð¾Ð´ÐºÐ°Ñ‚Ð°Ð»Ð¾Ð³`)
      const deepFile = fwd(tmp.path, "ç›®å½•", "Ð¿Ð¾Ð´ÐºÐ°Ñ‚Ð°Ð»Ð¾Ð³", "æ–‡ä»¶.txt")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...mod.map((file, i) => write(file, `before-${ids[i]}-Ã©\nðŸ™‚\nline`)),
    ...
    yield* Effect.all(
      [
        ...mod.map((file, i) => write(file, `after-${ids[i]}-Ã©\nðŸš€\nline`)),
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      expect(m!.patch).toContain(`-before-${ids[i]}-Ã©`)
      expect(m!.patch).toContain(`+after-${ids[i]}-Ã©`)
