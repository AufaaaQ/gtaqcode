...
import { SessionV1 } from "@gtaqcode/core/v1/session"
import { ModelsDev } from "@gtaqcode/core/models-dev"
import { HttpRecorder } from "@gtaqcode/http-recorder"
import { HttpRecorderInternal } from "@gtaqcode/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@gtaqcode/llm"
import { RequestExecutor } from "@gtaqcode/llm/route"
...
const zenURL = (connection: string) => `https://console.GTAQCODE.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.GTAQCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["GTAQCODE", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("GTAQCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("GTAQCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["GTAQCODE", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "GTAQCODE-proxy",
    name: "GTAQCODE proxy",
    providerID: ProviderV2.ID.GTAQCODE,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["GTAQCODE", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.GTAQCODE_RECORD_CONSOLE_TOKEN && process.env.GTAQCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.GTAQCODE,
        name: "GTAQCODE Zen",
        env: ["GTAQCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.GTAQCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.GTAQCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.GTAQCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["GTAQCODE", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("GTAQCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("GTAQCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("GTAQCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set GTAQCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the GTAQCODE LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "GTAQCODE.json"),
      JSON.stringify({ $schema: "https://GTAQCODE.ai/config.json", ...scenario.config(model) }),
    ),
  )
