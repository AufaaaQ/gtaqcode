...
import { LLMEvent, ToolFailure } from "@gtaqcode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@gtaqcode/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("GTAQCODE"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("GTAQCODE") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("GTAQCODE") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, GTAQCODE, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored GTAQCODE auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("GTAQCODE") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("GTAQCODE"),
