...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".GTAQCODE")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .GTAQCODE/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const GTAQCODE = path.join(test.directory, ".GTAQCODE")
      const tool = path.join(GTAQCODE, "tool")
  ...
  it.instance("ignores non-tool exports in .GTAQCODE/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".GTAQCODE", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` â€” `oh-my-GTAQCODE` was registering a tool
  ...
  it.instance("loads tools from .GTAQCODE/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const GTAQCODE = path.join(test.directory, ".GTAQCODE")
      const tools = path.join(GTAQCODE, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".GTAQCODE", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const GTAQCODE = path.join(test.directory, ".GTAQCODE")
        const customTools = path.join(GTAQCODE, "tools")
        const plugin = path.join(GTAQCODE, "node_modules", "@GTAQCODE-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(GTAQCODE, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@gtaqcode/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@gtaqcode/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".GTAQCODE", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(GTAQCODE, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@gtaqcode/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(GTAQCODE, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(GTAQCODE, "node_modules", "cowsay")
