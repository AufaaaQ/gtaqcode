...
import type { Event, GTAQCODEClient } from "@gtaqcode/sdk/v2"
...
type PermissionReplyParams = Parameters<GTAQCODEClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "GTAQCODE-acp-permission-"))
