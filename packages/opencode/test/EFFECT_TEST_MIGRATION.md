...
- `it.effect(...)` â€” pure Effect behavior with `TestClock` and
  `TestConsole`.
- `it.instance(...)` â€” service behavior that needs one scoped GTAQCODE
  instance.
...
9. Run the focused test file and `bun typecheck` from `packages/GTAQCODE`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/GTAQCODE/test
```
