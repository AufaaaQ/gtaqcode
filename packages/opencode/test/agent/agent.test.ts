...
it.instance("plan agent denies edits except .GTAQCODE/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".GTAQCODE/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".GTAQCODE", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.GTAQCODE_TEST_HOME
      process.env.GTAQCODE_TEST_HOME = test.directory
