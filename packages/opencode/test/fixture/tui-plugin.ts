...
import type { HostPluginApi } from "@gtaqcode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createGTAQCODEClient({
  ...
  let selected = opts.theme?.selected ?? "GTAQCODE"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "GTAQCODE.default",
