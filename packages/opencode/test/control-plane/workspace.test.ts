...
import { EventSequenceTable } from "@gtaqcode/core/event/sql"
...
import { WorkspaceV2 } from "@gtaqcode/core/workspace"
import { WorkspaceTable } from "@gtaqcode/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@gtaqcode/core/ripgrep"
import { AppNodeBuilder } from "@gtaqcode/core/effect/app-node-builder"
import { LayerNode } from "@gtaqcode/core/effect/layer-node"

const originalEnv = {
  GTAQCODE_AUTH_CONTENT: process.env.GTAQCODE_AUTH_CONTENT,
  GTAQCODE_EXPERIMENTAL_WORKSPACES: process.env.GTAQCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.GTAQCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@GTAQCODE.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.GTAQCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=GTAQCODE-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.GTAQCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.GTAQCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.GTAQCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=GTAQCODE-test")
