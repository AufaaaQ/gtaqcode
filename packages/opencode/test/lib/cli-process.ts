// Subprocess test harness for the GTAQCODE CLI. Spawns the real binary against
...
//
// Configuration flows through GTAQCODE's built-in test affordances:
//   - GTAQCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - GTAQCODE_TEST_HOME           : pins os.homedir() â†’ tmpdir
//   - GTAQCODE_DISABLE_PROJECT_CONFIG : skip walking up for GTAQCODE.json
//   - GTAQCODE_PURE                : skip external plugin discovery + install
//   - GTAQCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `GTAQCODE.run` is fully wired. The shape supports adding more
// builders (`GTAQCODE.serve(opts)`, `GTAQCODE.acp(opts)`, `GTAQCODE.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape â€” see the TODO at the bottom of GTAQCODECli.
...
import { AppProcess } from "@gtaqcode/core/process"
...
const GTAQCODERoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(GTAQCODERoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    GTAQCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    GTAQCODE_CONFIG_CONTENT: configJson,
    GTAQCODE_DISABLE_PROJECT_CONFIG: "1",
    GTAQCODE_PURE: "1",
    GTAQCODE_DISABLE_AUTOUPDATE: "1",
    GTAQCODE_DISABLE_AUTOCOMPACT: "1",
    GTAQCODE_DISABLE_MODELS_FETCH: "1",
    GTAQCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `GTAQCODE run`. New flags should
...
// `GTAQCODE serve` is a long-lived process â€” it never exits on its own.
...
// `GTAQCODE acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type GTAQCODECli = {
  ...
  // Spawn `GTAQCODE serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `GTAQCODE acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("GTAQCODE.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          GTAQCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("GTAQCODE.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("GTAQCODE.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "GTAQCODE server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `GTAQCODE serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("GTAQCODE.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const GTAQCODE: GTAQCODECli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, GTAQCODE })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `GTAQCODE.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "GTAQCODE") {
...
// every call site. Use this for any test that needs the GTAQCODE CLI fixture.
//
...
// resources (e.g. `GTAQCODE.serve`) without an extra `Effect.scoped` wrapper â€”
