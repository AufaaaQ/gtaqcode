...
import type { LLMClientShape } from "@gtaqcode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("GTAQCODE"))
    return { type: "unsupported", reason: "provider is not openai, GTAQCODE, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @gtaqcode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains GTAQCODE-owned. The native runtime only adapts
      // the @gtaqcode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
