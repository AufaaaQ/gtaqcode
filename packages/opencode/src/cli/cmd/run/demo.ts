...
import type { Event, ToolPart } from "@gtaqcode/sdk/v2"
...
const SAMPLE_TABLE = [
  ...
  "| Unicode | `æ¼¢å­—` | Wide characters should remain aligned |",
...
function emitTask(state: State): void {
  ...
  const part = {
    ...
    state: {
      status: "running",
      input: {
        filePath: "packages/GTAQCODE/src/cli/cmd/run/stream.ts",
        offset: 1,
        limit: 200,
      },
      time: {
  ...
  if (kind === "read") {
    ...
    askPermission(state, {
      ...
      always: [target],
        ...
        title: "read",
        output: ["1: {", '2:   "name": "GTAQCODE",', '3:   "private": true', "4: }"].join("\n"),
        metadata: {},
      },
    })
    return
