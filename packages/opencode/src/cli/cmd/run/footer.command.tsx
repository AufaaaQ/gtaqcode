...
export function RunQueuedPromptSelectBody(props: {
  ...
  const entries = createMemo<QueuedEntry[]>(() =>
    props.prompts().map((prompt) => ({
      ...
      footer: "queued Â· ctrl+e edit Â· ctrl+d remove",
...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "current"
              : model.cost?.input === 0 && provider.id === "GTAQCODE"
                ? "Free"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider = Number(a.providerID !== "GTAQCODE") - Number(b.providerID !== "GTAQCODE")
        if (provider !== 0) {
          return provider
        }
