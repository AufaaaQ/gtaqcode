// Shared type vocabulary for the direct interactive mode (`GTAQCODE --mini`).
//
...
import type { GTAQCODEClient, PermissionRequest, QuestionRequest, ToolPart } from "@gtaqcode/sdk/v2"
import type { TuiConfig } from "@gtaqcode/tui/config"
...
type PromptModel = Parameters<GTAQCODEClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<GTAQCODEClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<GTAQCODEClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<GTAQCODEClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<GTAQCODEClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<GTAQCODEClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<GTAQCODEClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<GTAQCODEClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<GTAQCODEClient["question"]["reject"]>[0]
