...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall GTAQCODE and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall GTAQCODE")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g GTAQCODE-ai",
      pnpm: "pnpm uninstall -g GTAQCODE-ai",
      bun: "bun remove -g GTAQCODE-ai",
      yarn: "yarn global remove GTAQCODE-ai",
      brew: "brew uninstall GTAQCODE",
      choco: "choco uninstall GTAQCODE",
      scoop: "scoop uninstall GTAQCODE",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "GTAQCODE", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using GTAQCODE!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# GTAQCODE") || content.includes(".GTAQCODE/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".GTAQCODE/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".GTAQCODE/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".GTAQCODE"))
    ) {
      continue
    }
