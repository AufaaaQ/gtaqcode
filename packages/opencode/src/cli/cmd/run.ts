import type { PermissionV1 } from "@gtaqcode/core/v1/permission"
import { FSUtil } from "@gtaqcode/core/fs-util"
// CLI entry point for `GTAQCODE run` and `GTAQCODE --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`GTAQCODE --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`GTAQCODE --mini --attach`): connects to a running
//      GTAQCODE server and runs interactive mode against it.
//
...
import { createGTAQCODEClient, type GTAQCODEClient, type ToolPart } from "@gtaqcode/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<GTAQCODEClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run GTAQCODE with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running GTAQCODE server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to GTAQCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to GTAQCODE_SERVER_USERNAME or 'GTAQCODE')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: GTAQCODEClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: GTAQCODEClient, sessionID: string) {
      ...
      async function current(sdk: GTAQCODEClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: GTAQCODEClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: GTAQCODEClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: GTAQCODEClient) {
        ...
        async function loop(client: GTAQCODEClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createGTAQCODEClient({
        baseUrl: "http://GTAQCODE.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
