...
import { GTAQCODEHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for GTAQCODE serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(GTAQCODEHttpApi)
...
export const PublicApi = GTAQCODEHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "GTAQCODE",
    version: "1.0.0",
    description: "GTAQCODE api",
