...
import { isRecord } from "@gtaqcode/tui/util/record"
...
import { TuiKeybind } from "@gtaqcode/tui/config/keybind"
...
import { TuiConfig } from "@gtaqcode/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@GTAQCODE/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old GTAQCODE.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.GTAQCODE`
  // folders between cwd and home, and GTAQCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.GTAQCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit GTAQCODE_TUI_CONFIG override, if set.
  if (Flag.GTAQCODE_TUI_CONFIG) {
    const configFile = Flag.GTAQCODE_TUI_CONFIG
  ...
  // 4. `.GTAQCODE` directories (and GTAQCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".GTAQCODE") || dir === Flag.GTAQCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".GTAQCODE") && dir !== Flag.GTAQCODE_CONFIG_DIR) continue
