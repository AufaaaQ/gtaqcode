...
const TUI_SCHEMA_URL = "https://GTAQCODE.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from GTAQCODE.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const GTAQCODE = await GTAQCODEFiles(input)
  for (const file of GTAQCODE) {
...
async function GTAQCODEFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "GTAQCODE"),
    ...(await Filesystem.findUp(["GTAQCODE.json", "GTAQCODE.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "GTAQCODE"))
  }
  if (Flag.GTAQCODE_CONFIG) files.push(Flag.GTAQCODE_CONFIG)
