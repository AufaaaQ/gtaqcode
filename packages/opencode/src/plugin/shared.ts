...
export const DEPRECATED_PLUGIN_PACKAGES = ["GTAQCODE-openai-codex-auth", "GTAQCODE-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, GTAQCODEVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(GTAQCODEVersion) || semver.major(GTAQCODEVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.GTAQCODE
  if (typeof range !== "string") return
  if (!semver.satisfies(GTAQCODEVersion, range)) {
    throw new Error(`Plugin requires GTAQCODE ${range} but running ${GTAQCODEVersion}`)
  }
}
