...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@gtaqcode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@gtaqcode/core/effect/app-node-builder"
import { errorData, errorMessage } from "@gtaqcode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@gtaqcode/tui/context/theme"
import { Global } from "@gtaqcode/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@gtaqcode/core/util/flock"
import { Flag } from "@gtaqcode/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@gtaqcode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@gtaqcode/core/v1/config/plugin"
import { createCommandShim } from "@gtaqcode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@gtaqcode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".GTAQCODE"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".GTAQCODE", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".GTAQCODE", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.GTAQCODE_PURE ? [] : pluginOrigins
    if (Flag.GTAQCODE_PURE && pluginOrigins.length) {
    }
