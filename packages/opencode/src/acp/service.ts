...
import type { AssistantMessage, Message, GTAQCODEClient, SessionMessageResponse } from "@gtaqcode/sdk/v2"
...
export const AuthMethodID = "GTAQCODE-login"
...
export class Service extends Context.Service<Service, Interface>()("@GTAQCODE/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `GTAQCODE auth login` in the terminal",
      name: "Login with GTAQCODE",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: GTAQCODEClient) {
...
function makeUsageService(sdk: GTAQCODEClient) {
...
async function loadDirectorySnapshot(sdk: GTAQCODEClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, GTAQCODE provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const GTAQCODEProvider = providers[ProviderV2.ID.make("GTAQCODE")]
  const GTAQCODEModel = GTAQCODEProvider ? Provider.sort(Object.values(GTAQCODEProvider.models))[0] : undefined
  if (GTAQCODEProvider && GTAQCODEModel) return { providerID: GTAQCODEProvider.id, modelID: GTAQCODEModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "GTAQCODE service failure", service })
}
