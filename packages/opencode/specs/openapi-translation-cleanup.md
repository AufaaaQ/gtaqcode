...
Trim `packages/GTAQCODE/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/GTAQCODE/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/GTAQCODE`.
- `[x]` `bun typecheck` from `packages/GTAQCODE`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/GTAQCODE/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/GTAQCODE`.
- `[x]` `bun dev generate > /tmp/GTAQCODE-openapi.json` from `packages/GTAQCODE`.
...
- Edit `packages/GTAQCODE/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/GTAQCODE-openapi.json` from `packages/GTAQCODE`.
...
- Check source schemas in files like `packages/GTAQCODE/src/session/schema.ts`, `packages/GTAQCODE/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/GTAQCODE/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/GTAQCODE/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
