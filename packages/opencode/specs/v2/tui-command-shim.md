...
Delete `packages/GTAQCODE/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/GTAQCODE/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/GTAQCODE/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/GTAQCODE`
- TUI plugin loader tests in `packages/GTAQCODE` if runtime plugin API wiring changed
