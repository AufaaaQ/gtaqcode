...
Practical reference for a future `packages/server` split after the GTAQCODE
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/GTAQCODE`.
...
Until enough shared service code lives outside `packages/GTAQCODE`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/GTAQCODE`

It should not import `packages/GTAQCODE` services while `packages/GTAQCODE`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on GTAQCODE-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/GTAQCODE` implementation details.
