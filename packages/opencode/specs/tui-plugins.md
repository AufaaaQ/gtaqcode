...
- Author package entrypoint is `@gtaqcode/plugin/tui`.
...
```json
{
  "$schema": "https://GTAQCODE.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/GTAQCODE-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@gtaqcode/plugin/tui`.
- `@gtaqcode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@gtaqcode/plugin/tui"
...
- The value is a semver range checked against the running GTAQCODE version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.GTAQCODE` is absent, no check is performed (backward compatible).
...
- `GTAQCODE plugin <module>` and TUI install both run install â†’ manifest read â†’ config patch.
- Alias: `GTAQCODE plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.GTAQCODE` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.GTAQCODE`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.GTAQCODE`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.GTAQCODE` dir or `GTAQCODE_CONFIG_DIR`, GTAQCODE installs `@gtaqcode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@gtaqcode/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus GTAQCODE's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
GTAQCODE registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, GTAQCODE pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@gtaqcode/plugin/tui`.
...
- `message` is required; `title` defaults to `"GTAQCODE"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `GTAQCODE.default`.
...
- Local plugins persist installed themes under the local `.GTAQCODE/themes` area near the plugin config source.
...
- `--pure` / `GTAQCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.GTAQCODE/plugins/tui-smoke.tsx`
- Local vim plugin: `.GTAQCODE/plugins/tui-vim.tsx`
- Local smoke config: `.GTAQCODE/tui.json`
- Local smoke theme: `.GTAQCODE/plugins/smoke-theme.json`
