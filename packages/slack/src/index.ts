...
import { createGTAQCODE, type ToolPart } from "@gtaqcode/sdk"
...
console.log("ðŸš€ Starting GTAQCODE server...")
const GTAQCODE = await createGTAQCODE({
  port: 0,
})
console.log("âœ… GTAQCODE server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await GTAQCODE.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("ðŸ†• Creating new GTAQCODE session...")
    const { client, server } = GTAQCODE
    ...
    console.log("âœ… Created GTAQCODE session:", createResult.data.id)
  ...
  console.log("ðŸ“ Sending to GTAQCODE:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("ðŸ“¤ GTAQCODE response:", JSON.stringify(result, null, 2))
