...
This is the living design and status document for `@gtaqcode/codemode` and its existing V2 GTAQCODE adapter.
...
`packages/GTAQCODE/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
