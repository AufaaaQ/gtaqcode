...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("GTAQCODE-find")
  api.delete("GTAQCODE-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("GTAQCODE-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("GTAQCODE-find", new Highlight(...rest))
    return true
  }
