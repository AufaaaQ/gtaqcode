...
import { getGTAQCODEModeStack, GTAQCODE_BASE_MODE, GTAQCODEKeymapProvider, registerGTAQCODEKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerGTAQCODEKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when GTAQCODE mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getGTAQCODEModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getGTAQCODEModeStack(keymap).push("autocomplete")
