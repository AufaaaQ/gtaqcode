...
export const GTAQCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const GTAQCODE_MODE_KEY = "GTAQCODE.mode"

export const GTAQCODEKeymapProvider = KeymapProvider
export const useGTAQCODEKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type GTAQCODEModeStack = ReturnType<typeof createGTAQCODEModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, GTAQCODEModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createGTAQCODEModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(GTAQCODE_MODE_KEY, GTAQCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(GTAQCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(GTAQCODE_MODE_KEY, stack.at(-1)?.mode ?? GTAQCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? GTAQCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(GTAQCODE_MODE_KEY, undefined)
...
export function useGTAQCODEModeStack() {
  return getGTAQCODEModeStack(useGTAQCODEKeymap())
}

export function getGTAQCODEModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("GTAQCODE mode stack is not registered for this keymap")
  return value
}
...
export function registerGTAQCODEKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createGTAQCODEModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useGTAQCODEKeymap()
