import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@gtaqcode/plugin/tui"
...
import type { useGTAQCODEKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useGTAQCODEKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getGTAQCODEModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getGTAQCODEModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
