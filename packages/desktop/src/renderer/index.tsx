...
import type { UpdaterState } from "@gtaqcode/app/updater"
...
import "./styles.css"
import { Splash } from "@gtaqcode/ui/logo"
...
const deepLinkEvent = "GTAQCODE:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__GTAQCODE__ ??= {}
  const pending = window.__GTAQCODE__.deepLinks ?? []
  window.__GTAQCODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `GTAQCODE.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://GTAQCODE.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("GTAQCODE.global.dat").getItem("language")
