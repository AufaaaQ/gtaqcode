import type { WslDistroProbe, WslGTAQCODECheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  GTAQCODEChecks: Record<string, WslGTAQCODECheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextGTAQCODEChecks = { ...GTAQCODEChecks }
  delete nextDistroProbes[distro]
  delete nextGTAQCODEChecks[distro]
  return { distroProbes: nextDistroProbes, GTAQCODEChecks: nextGTAQCODEChecks }
}
