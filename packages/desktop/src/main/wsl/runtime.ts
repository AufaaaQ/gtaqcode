...
export async function installWslGTAQCODE(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://GTAQCODE.ai/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslGTAQCODE(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.GTAQCODE/bin/GTAQCODE" ]; then printf "%s\\n" "$HOME/.GTAQCODE/bin/GTAQCODE"; fi',
        distro,
        opts,
      )
    ).stdout,
