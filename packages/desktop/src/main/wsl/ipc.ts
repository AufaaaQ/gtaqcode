...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-GTAQCODE", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installGTAQCODE(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-GTAQCODE", unavailable)
