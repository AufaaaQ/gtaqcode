...
import { type WslCommandLine, resolveWslGTAQCODE, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const GTAQCODE = await resolveWslGTAQCODE(distro)
  if (!GTAQCODE) throw new Error(`GTAQCODE is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "GTAQCODE"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export GTAQCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export GTAQCODE_CLIENT=desktop",
    `export GTAQCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export GTAQCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(GTAQCODE)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
