...
import { expectGTAQCODEVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveGTAQCODE?: typeof resolveWslGTAQCODE
...
export function createWslServersController(
  ...
  const setGTAQCODECheck = (distro: string, check: WslGTAQCODECheck) => {
    setState({
      GTAQCODEChecks: {
        ...state.GTAQCODEChecks,
        [distro]: check,
      },
    })
  }

  const checkGTAQCODE = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveGTAQCODE ?? resolveWslGTAQCODE)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return GTAQCODECheck(distro, resolved, version, appVersion)
  }

  const refreshGTAQCODECheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setGTAQCODECheck(distro, await checkGTAQCODE(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const GTAQCODEChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.GTAQCODEChecks[distro])
        .map(async (distro) => [distro, await checkGTAQCODE(distro, opts)] as const),
    )
    if (GTAQCODEChecks.length) {
      setState({ GTAQCODEChecks: { ...state.GTAQCODEChecks, ...Object.fromEntries(GTAQCODEChecks) } })
    }
  }
  ...
  const refreshGTAQCODECheckBackground = (id: string, distro: string) => {
    void checkGTAQCODE(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setGTAQCODECheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl GTAQCODE check failed", { id, distro, message })
      })
  }

  const refreshGTAQCODEChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkGTAQCODE(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setGTAQCODECheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl GTAQCODE check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshGTAQCODECheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installGTAQCODE(name: string) {
      await runJob({ kind: "install-GTAQCODE", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslGTAQCODE(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "GTAQCODE installation failed")
        }
        await refreshGTAQCODECheck(name, { signal: abort.signal })
        expectGTAQCODEVersion(state.GTAQCODEChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.GTAQCODEChecks, distro) : {}),
      })
    },
...
function GTAQCODECheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "GTAQCODE is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "GTAQCODE is installed but could not run",
    }
  }
  return {
    distro,
