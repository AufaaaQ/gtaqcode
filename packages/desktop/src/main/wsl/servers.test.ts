...
let releaseGTAQCODEResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectGTAQCODEVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectGTAQCODEVersion("1.14.35", "1.16.2")).toThrow(
    "GTAQCODE update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating GTAQCODE", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.GTAQCODE/bin/GTAQCODE",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, GTAQCODEChecks: {} })
})
...
test("ignores stale background GTAQCODE checks after removing a WSL server", async () => {
  persistedServers = []
  releaseGTAQCODEResolve = undefined
  ...
  await waitFor(() => !!releaseGTAQCODEResolve)
  await controller.removeServer("wsl:Debian")
  releaseGTAQCODEResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().GTAQCODEChecks).toEqual({})
})

test("ignores stale startup GTAQCODE checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking GTAQCODE", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const GTAQCODE: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveGTAQCODE: async (distro) => {
      GTAQCODE.push(distro)
      return "/home/me/.GTAQCODE/bin/GTAQCODE"
    },
  })
  ...
  expect(GTAQCODE).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(GTAQCODE).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().GTAQCODEChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check GTAQCODE in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const GTAQCODE: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(GTAQCODE).toEqual(["Debian"])
  expect(Object.keys(controller.getState().GTAQCODEChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveGTAQCODE: async () => {
      await new Promise<void>((resolve) => {
        releaseGTAQCODEResolve = resolve
      })
      return "/home/me/.GTAQCODE/bin/GTAQCODE"
    },
  }
