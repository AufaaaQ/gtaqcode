...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.GTAQCODE.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `GTAQCODE.settings.dat` is special: it maps to the `GTAQCODE.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"GTAQCODE.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // GTAQCODE.settings.dat â†’ the electron settings store ("GTAQCODE.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "GTAQCODE.global.dat").
  const storeName = filename === "GTAQCODE.settings.dat" ? "GTAQCODE.settings" : filename
