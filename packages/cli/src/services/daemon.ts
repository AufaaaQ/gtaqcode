...
import { createGTAQCODEClient } from "@gtaqcode/sdk/v2/client"
import { ServerAuth } from "@gtaqcode/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createGTAQCODEClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@GTAQCODE/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createGTAQCODEClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createGTAQCODEClient({ baseUrl: connection.url, headers: connection.headers })
    })
