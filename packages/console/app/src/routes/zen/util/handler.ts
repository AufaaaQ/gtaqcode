...
import { and, Database, eq, isNull, lt, or, sql } from "@gtaqcode/console-core/drizzle/index.js"
import { KeyTable } from "@gtaqcode/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@gtaqcode/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@gtaqcode/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@gtaqcode/console-core/util/date.js"
import { Identifier } from "@gtaqcode/console-core/identifier.js"
import { Billing } from "@gtaqcode/console-core/billing.js"
import { Actor } from "@gtaqcode/console-core/actor.js"
import { WorkspaceTable } from "@gtaqcode/console-core/schema/workspace.sql.js"
import { ZenData } from "@gtaqcode/console-core/model.js"
import { Subscription } from "@gtaqcode/console-core/subscription.js"
import { BlackData } from "@gtaqcode/console-core/black.js"
import { UserTable } from "@gtaqcode/console-core/schema/user.sql.js"
import { ModelTable } from "@gtaqcode/console-core/schema/model.sql.js"
import { ProviderTable } from "@gtaqcode/console-core/schema/provider.sql.js"
...
import { LiteData } from "@gtaqcode/console-core/lite.js"
import { Resource } from "@gtaqcode/console-resource"
...
import { Workspace } from "@gtaqcode/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-GTAQCODE-session") ?? ""
    const requestId = input.request.headers.get("x-GTAQCODE-request") ?? ""
    const ocClient = input.request.headers.get("x-GTAQCODE-client") ?? ""
    const projectId = input.request.headers.get("x-GTAQCODE-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://GTAQCODE.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-GTAQCODE-request")
            headers.delete("x-GTAQCODE-session")
            headers.delete("x-GTAQCODE-project")
            headers.delete("x-GTAQCODE-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-GTAQCODE-endpoint-id")
        const resEndpointModelId = res.headers.get("x-GTAQCODE-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://GTAQCODE.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://GTAQCODE.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://GTAQCODE.ai/workspace/${authInfo.workspaceID}/members`
