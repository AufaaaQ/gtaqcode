...
const directory = "C:/GTAQCODE/ReviewOpenFile"
...
test("opens and searches project files inline", async ({ page }) => {
  ...
  await mockGTAQCODEServer(page, {
    ...
    provider: {
      ...
      connected: ["GTAQCODE"],
      default: { providerID: "GTAQCODE", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "GTAQCODE.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "GTAQCODE.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: false, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "GTAQCODE.window.browser.dat:tabs",
