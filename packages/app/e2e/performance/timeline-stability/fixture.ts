...
import { SessionStatusEvent } from "@gtaqcode/schema/session-status-event"
import { SessionV1 } from "@gtaqcode/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockGTAQCODEServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/GTAQCODE/TimelineStability"
...
export const model = { providerID: "GTAQCODE", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("GTAQCODE.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["GTAQCODE"],
    default: { providerID: "GTAQCODE", modelID: "claude-opus-4-6" },
  }
}
