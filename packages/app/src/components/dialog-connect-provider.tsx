import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@gtaqcode/sdk/v2/client"
...
import { List, type ListRef } from "@gtaqcode/ui/list"
import { ProviderIcon } from "@gtaqcode/ui/provider-icon"
import { Spinner } from "@gtaqcode/ui/spinner"
import { Tag } from "@gtaqcode/ui/tag"
...
function ProviderPicker(props: { directory?: Accessor<string | undefined>; onSelect: (provider: string) => void }) {
  ...
  const note = (id: string) => {
    ...
    if (id === "GTAQCODE-go") return language.t("dialog.provider.GTAQCODEGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "GTAQCODE"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.GTAQCODE.tagline")}</div>
          </Show>
  ...
  function ApiAuthView() {
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "GTAQCODE"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.GTAQCODEZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.GTAQCODEZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.GTAQCODEZen.visit.prefix")}
                <Link href="https://GTAQCODE.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.GTAQCODEZen.visit.link")}
                </Link>
                {language.t("provider.connect.GTAQCODEZen.visit.suffix")}
              </div>
            </div>
          </Match>
          <Match when={true}>
