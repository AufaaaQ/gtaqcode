...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when GTAQCODE needs attention", () => {
    expect(wslGTAQCODEAction(undefined)).toBeUndefined()
    expect(
      wslGTAQCODEAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install GTAQCODE")
    expect(
      wslGTAQCODEAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/GTAQCODE",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update GTAQCODE")
    expect(
      wslGTAQCODEAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/GTAQCODE",
  ...
  test("does not report ready when GTAQCODE is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        GTAQCODEChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.GTAQCODE/bin/GTAQCODE",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installGTAQCODE" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-GTAQCODE")
  })
