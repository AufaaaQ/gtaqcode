...
import { Tag } from "@gtaqcode/ui/v2/badge-v2"
...
import { wslGTAQCODEAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.GTAQCODEChecks[item.config.distro]
          const GTAQCODEAction = () => wslGTAQCODEAction(check())
          const busy = () => wsl.data?.job?.kind === "install-GTAQCODE" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={GTAQCODEAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installGTAQCODE(item.config.distro))}
                    >
