...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-GTAQCODE" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslGTAQCODEAction(check?: WslGTAQCODECheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install GTAQCODE"
  if (check.matchesDesktop === false) return "Update GTAQCODE"
}
...
export function addServerViewModel(input: {
  ...
  const GTAQCODECheck = selectedDistro ? (state?.GTAQCODEChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.GTAQCODEChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateGTAQCODE" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.GTAQCODEMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installGTAQCODE" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  GTAQCODECheck: WslGTAQCODECheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingGTAQCODE =
    input.state?.job?.kind === "install-GTAQCODE" && input.state.job.distro === input.selectedDistro
  if (!ready || probingGTAQCODE) {
    return {
      variant: "contrast",
  ...
  if (!addServerGTAQCODEReady(input.GTAQCODECheck)) {
    const update = !!input.GTAQCODECheck?.resolvedPath && input.GTAQCODECheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingGTAQCODE
        ? { key: "wsl.onboarding.updatingGTAQCODE" }
        : update
          ? { key: "wsl.onboarding.updateGTAQCODE" }
          : { key: "wsl.onboarding.installGTAQCODE" },
      disabled: !!input.state?.job || input.adding,
      action: "install-GTAQCODE",
      loading: installingGTAQCODE,
...
function addServerGTAQCODEReady(check: WslGTAQCODECheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.GTAQCODEChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.GTAQCODEChecks[item.name]) return [`GTAQCODE:${item.name}`]
    return []
  })
