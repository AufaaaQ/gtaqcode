...
import { type Session } from "@gtaqcode/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("GTAQCODE://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("GTAQCODE://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("GTAQCODE://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("GTAQCODE://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("GTAQCODE://open-project")).toBeUndefined()
    expect(parseDeepLink("GTAQCODE://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "GTAQCODE://open-project?directory=/a",
      "GTAQCODE://other?directory=/b",
      "GTAQCODE://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("GTAQCODE://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("GTAQCODE://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("GTAQCODE://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("GTAQCODE://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "GTAQCODE://new-session?directory=/a",
      "GTAQCODE://open-project?directory=/b",
      "GTAQCODE://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __GTAQCODE__: {
        deepLinks: ["GTAQCODE://open-project?directory=/a"],
      },
    } as unknown as Window & { __GTAQCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["GTAQCODE://open-project?directory=/a"])
