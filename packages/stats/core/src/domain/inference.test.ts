...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve GTAQCODE route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "GTAQCODE")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "GTAQCODE")).toBe("openai")
    expect(statProvider("big-pickle", "", "GTAQCODE")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "GTAQCODE"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep GTAQCODE as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "GTAQCODE"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "GTAQCODE"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep GTAQCODE or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "GTAQCODE"), country: "US" })).toMatchObject([
