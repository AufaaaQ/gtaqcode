...
// test/preload.ts pins GTAQCODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.GTAQCODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.GTAQCODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.GTAQCODE_MODELS_PATH = undefined
  Flag.GTAQCODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.GTAQCODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.GTAQCODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.GTAQCODE_DISABLE_MODELS_FETCH = false
        }),
