...
import { GoogleVertexPlugin } from "@gtaqcode/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.GTAQCODE, (provider) => {
          provider.api = {
            ...
            url: "https://GTAQCODE.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.GTAQCODE))
