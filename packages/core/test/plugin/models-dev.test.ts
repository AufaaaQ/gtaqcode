...
import { ModelsDevPlugin } from "@gtaqcode/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.GTAQCODE_MODELS_PATH,
          disabled: Flag.GTAQCODE_DISABLE_MODELS_FETCH,
        }
        Flag.GTAQCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.GTAQCODE_MODELS_PATH = previous.path
          Flag.GTAQCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
