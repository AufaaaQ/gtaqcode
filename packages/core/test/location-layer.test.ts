...
import { define } from "@gtaqcode/plugin/v2/effect"
...
import { LocationServiceMap } from "@gtaqcode/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "GTAQCODE.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "GTAQCODE.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
