...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@GTAQCODE/acme")).toBe("@GTAQCODE/acme")
    expect(Npm.sanitize("@GTAQCODE/acme@1.0.0")).toBe("@GTAQCODE/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/GTAQCODE/acme.git"
    const expected = win ? "acme@git+https_//github.com/GTAQCODE/acme.git" : spec
