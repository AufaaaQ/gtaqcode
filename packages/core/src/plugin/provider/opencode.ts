...
import type { IntegrationOAuthMethodRegistration } from "@gtaqcode/plugin/v2/effect/integration"
import { define } from "@gtaqcode/plugin/v2/effect/plugin"
import type { CredentialValue } from "@gtaqcode/sdk/v2/types"
...
const defaultServer = "https://console.GTAQCODE.ai"
const clientID = "GTAQCODE-cli"
...
export const GTAQCODEPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "GTAQCODE",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("GTAQCODEPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("GTAQCODE")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load GTAQCODE provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("GTAQCODE", (integration) => {
        integration.name = "GTAQCODE"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "GTAQCODE", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("GTAQCODE")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("GTAQCODE")
      ...
      const item = catalog.provider.get(ProviderV2.ID.GTAQCODE)
      if (!item) return
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("GTAQCODE")),
