...
import { ToolFailure } from "@gtaqcode/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@GTAQCODE/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.GTAQCODE_WEBSEARCH_PROVIDER === "exa" || process.env.GTAQCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.GTAQCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("GTAQCODE_EXPERIMENTAL") || truthy("GTAQCODE_ENABLE_EXA") || truthy("GTAQCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("GTAQCODE_ENABLE_PARALLEL") || truthy("GTAQCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `GTAQCODE/${InstallationVersion}`,
