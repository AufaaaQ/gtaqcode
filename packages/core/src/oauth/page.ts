...
// etc.).
//
// The visual language mirrors the GTAQCODE app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `GTAQCODE is now connected to ${escapeHtml(provider)}.` : "GTAQCODE is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `GTAQCODE couldn't finish connecting to ${escapeHtml(provider)}.`
        : "GTAQCODE couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from GTAQCODE.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} Â· GTAQCODE</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("GTAQCODE couldn't finish connecting to "+PROVIDER+"."):"GTAQCODE couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from GTAQCODE."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("GTAQCODE is now connected to "+PROVIDER+"."):"GTAQCODE is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// GTAQCODE wordmark â€” same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="GTAQCODE" role="img">
