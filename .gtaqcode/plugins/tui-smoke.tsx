...
import type { TuiPlugin, TuiPluginApi, TuiPluginMeta, TuiPluginModule, TuiSlotPlugin } from "@gtaqcode/plugin/tui"
...
const host = (api: TuiPluginApi, input: Cfg, skin: Skin) => {
  ...
  api.ui.dialog.replace(() => (
    <box paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1} flexDirection="column">
      ...
      <text fg={skin.muted}>esc closes Â· depth {api.ui.dialog.depth}</text>
    ...
    <box width={dim().width} height={dim().height} backgroundColor={skin.panel} position="relative">
        ...
        >
          {value.tab === 0 ? (
            <box flexDirection="column" gap={1}>
              ...
              <text fg={skin.muted}>
                first: {props.meta.state === "first" ? "yes" : "no"} Â· updated:{" "}
                {props.meta.state === "updated" ? "yes" : "no"} Â· loads: {props.meta.load_count}
              </text>
          ...
          {value.tab === 2 ? (
            <box flexDirection="column" gap={1}>
              ...
              <text fg={skin.muted}>
                local open: {shortcuts().local_push} push nested Â· {shortcuts().local_pop} close
              </text>
      ...
      >
        ...
        >
          ...
          <text fg={skin.muted}>
            {shortcuts().local_push} push nested Â· {shortcuts().local_pop} pop/close
          </text>
          <box flexDirection="row" gap={1}>
    ...
    <box width="100%" height="100%" backgroundColor={skin.panel}>
      <Dialog onClose={() => props.api.route.navigate("home")}>
        <box paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1} flexDirection="column">
          ...
          <text fg={skin.muted}>
            {shortcuts().modal_accept} opens screen Â· {shortcuts().modal_close} closes
          </text>
          <box flexDirection="row" gap={1}>
            <Btn
              txt="open screen"
    ...
    sidebar_content(ctx, value) {
      ...
      return (
        ...
        >
          ...
          <text fg={skin.muted}>
            {input.label} order {order} Â· session {value.session_id.slice(0, 8)}
          </text>
        </box>
      )
    },
